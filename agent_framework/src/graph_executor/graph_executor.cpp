...
std::string_view execution_event_kind(ExecutionEventType type) {
    switch (type) {
        case ExecutionEventType::TaskSubmitted: return "task_submitted";
        case ExecutionEventType::TaskStarted: return "task_started";
        case ExecutionEventType::TaskStatusChanged: return "task_status_changed";
        case ExecutionEventType::ToolStarted: return "tool_started";
        case ExecutionEventType::ToolCompleted: return "tool_completed";
        case ExecutionEventType::ChildTaskUpdated: return "child_task_updated";
        case ExecutionEventType::VerifierStarted: return "verifier_started";
        case ExecutionEventType::VerifierCompleted: return "verifier_completed";
        case ExecutionEventType::MemoryAssembled: return "memory_assembled";
        case ExecutionEventType::MemoryEvicted: return "memory_evicted";
        case ExecutionEventType::MemoryCompacted: return "memory_compacted";
        case ExecutionEventType::ArtifactUpdated: return "artifact_updated";
        case ExecutionEventType::CheckpointCommitted: return "checkpoint_committed";
        case ExecutionEventType::ExecutionCompleted: return "execution_completed";
    }
    return "unknown";
}

std::string_view execution_event_component(ExecutionEventType type) {
    switch (type) {
        case ExecutionEventType::ToolStarted:
        case ExecutionEventType::ToolCompleted: return "tool";
        case ExecutionEventType::ChildTaskUpdated: return "child_task";
        case ExecutionEventType::VerifierStarted:
        case ExecutionEventType::VerifierCompleted: return "verifier";
        case ExecutionEventType::MemoryAssembled:
        case ExecutionEventType::MemoryEvicted:
        case ExecutionEventType::MemoryCompacted: return "memory";
        case ExecutionEventType::ArtifactUpdated: return "artifact";
        case ExecutionEventType::CheckpointCommitted: return "session_store";
        default: return "graph_executor";
    }
}

json audit_projection(const ExecutionEvent& event) {
    json payload = redact_audit_payload(event.payload);
    if (!payload.is_object()) return {{"payload_digest", audit_payload_digest(payload)},
                                      {"payload_bytes", payload.dump().size()}};
    const auto summarize = [&](const char* key) {
        if (!payload.contains(key)) return;
        const auto serialized = payload[key].dump();
        payload[std::string(key) + "_digest"] = audit_payload_digest(payload[key]);
        payload[std::string(key) + "_bytes"] = serialized.size();
        payload.erase(key);
    };
    summarize("arguments");
    summarize("result");
...
ExecutionResult GraphExecutor::execute_sync(tf::Executor& executor, ExecutionRequest request) {
    ...
    std::vector<std::string> started_tool_effect_keys;
    std::vector<std::string> completed_tool_effect_keys;
    std::mutex tool_effect_mutex;
    ...
    const std::string audit_trace_id = request.context.trace_id.empty() ? run_id : request.context.trace_id;
    const std::string audit_tenant_id = request.context.tenant_id;
    ExecutionEventEmitter emitter([public_event_sink, audit_sink, audit_attempt,
                                   audit_trace_id, audit_tenant_id](const ExecutionEvent& event) {
        if (public_event_sink) public_event_sink(event);
        if (audit_sink) {
            AuditEvent audit;
            audit.timestamp = event.timestamp;
            audit.trace_id = audit_trace_id;
            audit.tenant_id = audit_tenant_id;
            audit.session_id = event.session_id;
            audit.task_id = event.task_id;
            audit.attempt = audit_attempt;
            audit.component = execution_event_component(event.type);
            audit.event_kind = execution_event_kind(event.type);
            audit.outcome = event.type == ExecutionEventType::ExecutionCompleted
                ? (event.payload.value("success", false) ? "completed" : "failed") : "observed";
            audit.sequence = event.sequence;
            audit.source_sequence = event.sequence;
            audit.payload = audit_projection(event);
    ...
    request.options.react.graph_options.tool_execution_observer =
        [&, prior_tool_observer](const ToolExecutionEvent& event) {
            if (request.tool_effect_journal) {
                const std::string request_digest = stable_result_digest(event.arguments);
                const std::string event_identity = !event.tool_call_id.empty() ? event.tool_call_id
                    : event.tool_name + ":" + request_digest;
                std::lock_guard<std::mutex> effect_lock(tool_effect_mutex);
                if (event.phase == ToolExecutionPhase::Started) {
                    ToolSideEffect side_effect = ToolSideEffect::Unknown;
                    try {
                        if (request.deps.toolbus)
                            side_effect = request.deps.toolbus->get_tool_meta(event.tool_name).side_effect;
                    } catch (...) {}
                    const bool read_only = side_effect == ToolSideEffect::ReadOnly;
                    std::string explicit_key;
                    if (event.arguments.is_object()) {
                        for (const char* field : {"_af_idempotency_key", "idempotency_key"}) {
                            if (event.arguments.contains(field) && event.arguments[field].is_string()) {
                                explicit_key = event.arguments[field].get<std::string>();
                                if (!explicit_key.empty()) break;
                            }
                        }
                    }
                    const std::string key = session_id + ":" +
                        (!explicit_key.empty() ? explicit_key : event_identity);
                    ToolEffectRecord effect;
                    effect.task_id = request.context.task_id.value_or("");
                    effect.session_id = session_id;
                    effect.tool_name = event.tool_name;
                    ...
                    effect.safe_to_replay = read_only;
                    effect.reconciliation_policy = read_only
                        ? ToolReconciliationPolicy::ReplayIdempotent
                        : explicit_key.empty() ? ToolReconciliationPolicy::ManualReview
                                               : ToolReconciliationPolicy::LookupExternal;
                    const auto begin = request.tool_effect_journal->begin(effect);
                    if (!read_only && explicit_key.empty()) {
                        if (begin == ToolEffectBeginResult::Started)
                            request.tool_effect_journal->mark_manual_review(
                                key, "explicit_idempotency_key_required");
                        throw ToolEffectBlocked("tool_effect_idempotency_key_required");
                    }
                    if (begin == ToolEffectBeginResult::ExistingInFlight && read_only) {
                        const auto previous = request.tool_effect_journal->find_idempotency(key);
                        if (!previous || request.tool_effect_journal->reconciliation_action(*previous) !=
                                             ToolReconciliationAction::Replay ||
                            !request.tool_effect_journal->prepare_replay(key, expected_revision + 1))
                            throw ToolEffectBlocked("tool_effect_reconciliation_required");
                    } else if (begin != ToolEffectBeginResult::Started) {
                        throw ToolEffectBlocked(begin == ToolEffectBeginResult::Conflict
                            ? "tool_effect_idempotency_conflict" : "tool_effect_duplicate_blocked");
                    }
                    tool_effect_keys[event_identity] = key;
                    started_tool_effect_keys.push_back(key);
                } else {
                    const auto it = tool_effect_keys.find(event_identity);
    ...
    if (!wr.success || cancelled || deadline_exceeded) {
        ...
        if (request.tool_effect_journal) {
            std::lock_guard<std::mutex> effect_lock(tool_effect_mutex);
            for (const auto& key : started_tool_effect_keys) {
                const auto record = request.tool_effect_journal->find_idempotency(key);
                if (record && record->status == ToolEffectStatus::Started) {
                    if (cancelled || deadline_exceeded)
                        request.tool_effect_journal->cancel(
                            key, deadline_exceeded ? "deadline_exceeded" : "cancelled");
                    else request.tool_effect_journal->fail(key, "execution_failed");
                }
            }
        }
