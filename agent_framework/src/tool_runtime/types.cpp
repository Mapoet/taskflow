...
namespace agent_framework::tool_runtime
{
    ...
    {
        static const std::set<std::string> fields = {"invocation_id", "conversation_id", "turn_id", "tool_call_id", "tool_name", "tool_contract_revision", "deployment_revision", "tool_generation", "input_digest", "revision", "attempt", "state", "budget", "lease", "progress_sequence", "checkpoint_ref", "checkpoint_digest", "created_at", "updated_at", "idempotent"};
        auto d = contracts::parse_typed_contract(j, "agent.long_running_tool_invocation/v1", c, issues);
        if (!d || !contracts::validate_object_fields(d->payload, fields, fields, c.unknown_fields, nullptr, issues, "/payload"))
            return {};
        try
        {
