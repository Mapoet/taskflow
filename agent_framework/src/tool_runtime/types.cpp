...
namespace agent_framework::tool_runtime
{
    ...
    std::optional<LongRunningToolInvocation> decode_invocation(const json &j, const contracts::ParseContext &c, std::vector<contracts::ContractIssue> *issues)
    {
        ...
        static auto fields = []
        {auto v=required;v.insert({"adapter_id","adapter_revision","adapter_generation","external_operation_id","provider_session_id","remote_task_id","adapter_peer_id","adapter_restart_policy"});return v; }();
        ...
        try
        {
            ...
            v.adapter_id = p.value("adapter_id", "");
            v.adapter_revision = p.value("adapter_revision", "");
            v.adapter_generation = p.value("adapter_generation", "");
            v.external_operation_id = p.value("external_operation_id", "");
            v.provider_session_id = p.value("provider_session_id", "");
            v.remote_task_id = p.value("remote_task_id", "");
            v.adapter_peer_id = p.value("adapter_peer_id", "");
            v.adapter_restart_policy = p.value("adapter_restart_policy", "");
