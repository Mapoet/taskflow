...
namespace agent_framework::agent_template
{
    namespace
    {
        ...
        std::string utc_now(){const auto now=std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());std::tm tm{};
#if defined(_WIN32)
            gmtime_s(&tm,&now);
#else
            gmtime_r(&now,&tm);
    ...
    RunnerResult ToolBusSkillRunner::run(const RunnerRequest &r)
    {
        ...
        const auto allowed = std::any_of(r.session.effective_permissions.tools.begin(), r.session.effective_permissions.tools.end(),
                                         [&](const auto &grant) { return grant == "*" || portable_tool_name(bus_->resolve_tool_name(grant)) == portable_tool; });
        if (!allowed)
        {
        ...
        out.ok = !out.output.contains("error") && !cancel->load();
        out.error_code = cancel->load() ? "cancelled" : out.ok ? ""
                                                               : "tool_failed";
    ...
    ChildTaskSkillRunner::ChildTaskSkillRunner(std::shared_ptr<ChildTaskBackend> backend):backend_(std::move(backend)){if(!backend_)throw std::invalid_argument("ChildTaskBackend required");}
    RunnerResult ChildTaskSkillRunner::run(const RunnerRequest&r){
        RunnerResult out;out.receipt.invocation_id=r.invocation.invocation_id;out.receipt.node_id=r.node.node_id;out.receipt.runner=kind();
        ChildTaskRequest request;request.child_id=r.node.node_id;request.parent_run_id=r.invocation.invocation_id;
        request.run_id=r.invocation.invocation_id+":"+r.node.node_id;request.idempotency_key=r.node.idempotency_key;request.inputs=r.input;
        request.policy.cancel_requested=r.cancel;request.grants.tools=r.session.effective_permissions.tools;
        request.grants.network=r.session.effective_permissions.network;request.grants.environment=r.session.effective_permissions.environment;
        request.grants.filesystem_read=r.session.effective_permissions.filesystem_read;request.grants.filesystem_write=r.session.effective_permissions.filesystem_write;request.grants.secrets=r.session.effective_permissions.secrets;
        out.events={event(r,1,RunnerLifecycleState::Admitted),event(r,2,RunnerLifecycleState::Prepared),event(r,3,RunnerLifecycleState::Running)};
        try{auto handle=backend_->start(std::move(request));if(!handle)throw std::runtime_error("child backend returned no handle");
            const auto key=r.invocation.invocation_id+":"+r.node.node_id;{std::lock_guard lock(mutex_);active_[key]=handle.get();}
            auto result=handle->wait();{std::lock_guard lock(mutex_);active_.erase(key);}out.output=child_task_result_to_json(result);out.ok=result.ok();
            out.error_code=result.error_code;out.error_message=result.error.value_or("");}
        catch(const std::exception&e){out.error_code="child_task_failed";out.error_message=e.what();}
        out.receipt.terminal_state=out.ok?RunnerLifecycleState::Succeeded:RunnerLifecycleState::Failed;out.receipt.output_digest=contracts::embedded_digest(out.output).value_or("");
        out.events.push_back(event(r,4,out.receipt.terminal_state,{{"output_digest",out.receipt.output_digest},{"error_code",out.error_code}}));return out;}
    bool ChildTaskSkillRunner::cancel(std::string_view id){std::lock_guard lock(mutex_);auto it=active_.find(std::string(id));if(it==active_.end())return false;it->second->cancel();return true;}
    NestedWorkflowSkillRunner::NestedWorkflowSkillRunner(std::shared_ptr<NestedWorkflowPort>port):port_(std::move(port)){if(!port_)throw std::invalid_argument("NestedWorkflowPort required");}
    ApprovalSkillRunner::ApprovalSkillRunner(approval::ApprovalStore&store):store_(store){}
    RunnerResult ApprovalSkillRunner::run(const RunnerRequest&r){RunnerResult out;out.receipt.invocation_id=r.invocation.invocation_id;out.receipt.node_id=r.node.node_id;out.receipt.runner=kind();out.events={event(r,1,RunnerLifecycleState::Admitted),event(r,2,RunnerLifecycleState::Prepared)};
        const auto id=r.input.value("approval_id",std::string{});if(id.empty()){out.error_code="approval_id_required";out.receipt.terminal_state=RunnerLifecycleState::Failed;return out;}
        auto request=store_.request(id);if(!request){out.error_code="approval_request_not_found";out.receipt.terminal_state=RunnerLifecycleState::Failed;return out;}
        auto decision=store_.latest_decision(id);if(!decision){out.error_code="awaiting_approval";out.error_message=id;out.receipt.terminal_state=RunnerLifecycleState::Waiting;out.receipt.checkpoint_ref="approval:"+id;out.events.push_back(event(r,3,RunnerLifecycleState::Waiting,{{"approval_id",id},{"checkpoint_ref",out.receipt.checkpoint_ref}}));return out;}
        ...
        if(decision->request_digest!=request_digest || (!r.invocation.plan_digest.empty()&&decision->plan_digest!=r.invocation.plan_digest) ||
           ...
           (!expected_policy.empty()&&decision->policy_revision!=expected_policy)){
            out.error_code="stale_or_scope_mismatched_approval";out.receipt.terminal_state=RunnerLifecycleState::Failed;
            out.events.push_back(event(r,3,RunnerLifecycleState::Failed,{{"approval_id",id},{"reason","binding_mismatch"}}));return out;}
        if((!request->expires_at.empty()&&utc_now()>request->expires_at)||(!decision->expires_at.empty()&&utc_now()>decision->expires_at)||
           decision->decision==approval::Decision::Expired||decision->decision==approval::Decision::Revoked){
            out.error_code="approval_expired_or_revoked";out.receipt.terminal_state=RunnerLifecycleState::Failed;return out;}
        out.output=approval::encode(*decision);out.ok=decision->decision==approval::Decision::Approved;out.error_code=out.ok?"":"approval_not_granted";out.receipt.terminal_state=out.ok?RunnerLifecycleState::Succeeded:RunnerLifecycleState::Failed;out.receipt.output_digest=contracts::embedded_digest(out.output).value_or("");out.events.push_back(event(r,3,out.receipt.terminal_state,{{"approval_id",id}}));return out;}
    ...
    std::shared_ptr<SkillRunnerRegistry> build_production_runners(ProductionRunnerDependencies dependencies)
    {
        ...
        if(dependencies.child_tasks)runners->register_runner(std::make_shared<ChildTaskSkillRunner>(dependencies.child_tasks));
        if(dependencies.nested_workflows)runners->register_runner(std::make_shared<NestedWorkflowSkillRunner>(dependencies.nested_workflows));
        if(dependencies.approvals)runners->register_runner(std::make_shared<ApprovalSkillRunner>(*dependencies.approvals));
        return runners;
    }
