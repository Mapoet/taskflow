...
public:
    ...
    void commit() { exec(db_, "COMMIT"); finished_ = true; }
private:
    sqlite3* db_;
...
void SQLiteRunStore::migrate() {
    ...
    exec(db, "CREATE TABLE IF NOT EXISTS run_schema_version("
             "version INTEGER PRIMARY KEY, applied_at TEXT NOT NULL)");
    int version = 0;
    {
        Statement query(db, "SELECT COALESCE(MAX(version),0) FROM run_schema_version");
    ...
    if(version == 0) {
        ...
        exec(db, "CREATE INDEX runs_recovery_idx ON runs(state,updated_at)");
        exec(db, "CREATE TABLE run_events("
        ...
        exec(db, "CREATE TABLE run_interruptions("
        ...
        exec(db, "CREATE TABLE graph_definitions("
        ...
        exec(db, "CREATE TABLE durable_timers("
        ...
        exec(db, "CREATE INDEX durable_timers_due_idx ON durable_timers(completed,due_unix_ms,lease_until_unix_ms)");
        exec(db, "INSERT INTO run_schema_version(version,applied_at) "
...
StoreResult SQLiteRunStore::create(const RunCheckpoint& initial) {
    ...
    Statement statement(db, "INSERT INTO runs(run_id,tenant_id,task_id,revision,state,checkpoint_json,"
                            "graph_revision,plan_digest) VALUES(?,?,?,?,?,?,?,?)");
    bind_text(statement.get(), 1, initial.metadata.identity.run_id);
    bind_text(statement.get(), 2, initial.metadata.identity.tenant_id);
    bind_text(statement.get(), 3, initial.metadata.identity.task_id);
    sqlite3_bind_int64(statement.get(), 4, 1);
    bind_text(statement.get(), 5, run_state_name(initial.state));
    bind_text(statement.get(), 6, document);
    bind_text(statement.get(), 7, initial.graph_revision);
    bind_text(statement.get(), 8, initial.plan_digest);
...
std::optional<RunRecord> SQLiteRunStore::load(std::string_view run_id) {
    std::lock_guard lock(mutex_);
    auto* db = database(db_);
    Statement statement(db, "SELECT revision,checkpoint_json,updated_at FROM runs WHERE run_id=?");
    bind_text(statement.get(), 1, run_id);
    if(sqlite3_step(statement.get()) != SQLITE_ROW) return std::nullopt;
    auto checkpoint = checkpoint_from_text(column_text(statement.get(), 1));
...
StoreResult SQLiteRunStore::checkpoint(const RunCheckpoint& next, std::uint64_t expected_revision) {
    ...
    try {
        ...
        {
            Statement query(db, "SELECT revision,checkpoint_json FROM runs WHERE run_id=?");
            bind_text(query.get(), 1, next.metadata.identity.run_id);
            ...
            auto prior = checkpoint_from_text(column_text(query.get(), 1));
        ...
        Statement update(db, "UPDATE runs SET revision=revision+1,state=?,checkpoint_json=?,"
                             "graph_revision=?,plan_digest=?,updated_at=strftime('%Y-%m-%dT%H:%M:%fZ','now') "
                             "WHERE run_id=? AND revision=?");
        bind_text(update.get(), 1, run_state_name(next.state));
        bind_text(update.get(), 2, encode(next).dump());
        bind_text(update.get(), 3, next.graph_revision);
        bind_text(update.get(), 4, next.plan_digest);
        bind_text(update.get(), 5, next.metadata.identity.run_id);
...
std::vector<RunRecord> SQLiteRunStore::list_recoverable(std::size_t limit) {
    ...
    auto* db = database(db_);
    Statement statement(db, "SELECT revision,checkpoint_json,updated_at FROM runs "
...
StoreResult SQLiteRunStore::append_event(const RunEvent& event) {
    ...
    try {
        ...
        if(sequence == 0) {
            Statement query(db, "SELECT COALESCE(MAX(sequence),0)+1 FROM run_events WHERE run_id=?");
            bind_text(query.get(), 1, event.run_id);
            if(sqlite3_step(query.get()) == SQLITE_ROW)
        ...
        Statement insert(db, "INSERT INTO run_events(run_id,sequence,event_type,payload_json,payload_digest)"
                             " VALUES(?,?,?,?,?)");
        ...
        bind_text(insert.get(), 4, event.payload.dump());
...
std::vector<RunEvent> SQLiteRunStore::events(std::string_view run_id,
    ...
    auto* db = database(db_);
    Statement statement(db, "SELECT sequence,event_type,payload_json,payload_digest,created_at "
    ...
    while(sqlite3_step(statement.get()) == SQLITE_ROW) {
        result.push_back({std::string(run_id),
            static_cast<std::uint64_t>(sqlite3_column_int64(statement.get(), 0)),
            column_text(statement.get(), 1), json::parse(column_text(statement.get(), 2)),
...
StoreResult SQLiteRunStore::put_interruption(const Interruption& interruption) {
    ...
    auto* db = database(db_);
    Statement statement(db, "INSERT INTO run_interruptions(interruption_id,run_id,token_digest,"
                            "document_json,expires_at) VALUES(?,?,?,?,?)");
    bind_text(statement.get(), 1, interruption.interruption_id);
    bind_text(statement.get(), 2, run_id);
    bind_text(statement.get(), 3, interruption.resume_token_digest);
    bind_text(statement.get(), 4, encode(interruption).dump());
    bind_text(statement.get(), 5, interruption.expires_at);
    const int rc = sqlite3_step(statement.get());
...
std::optional<Interruption> SQLiteRunStore::load_interruption(std::string_view interruption_id) {
    std::lock_guard lock(mutex_);
    auto* db = database(db_);
    Statement statement(db, "SELECT document_json FROM run_interruptions WHERE interruption_id=?");
    bind_text(statement.get(), 1, interruption_id);
    if(sqlite3_step(statement.get()) != SQLITE_ROW) return std::nullopt;
    auto value = decode_interruption(json::parse(column_text(statement.get(), 0)));
...
StoreResult SQLiteRunStore::consume_resume_token(std::string_view interruption_id,
    ...
    std::lock_guard lock(mutex_);
    auto* db = database(db_);
    Statement statement(db, "UPDATE run_interruptions SET consumed=1 WHERE interruption_id=? "
                            "AND run_id=? AND token_digest=? AND consumed=0 "
                            "AND (expires_at='' OR expires_at>strftime('%Y-%m-%dT%H:%M:%fZ','now'))");
    bind_text(statement.get(), 1, interruption_id);
    bind_text(statement.get(), 2, run_id);
    bind_text(statement.get(), 3, token_digest);
    const int rc = sqlite3_step(statement.get());
    if(rc != SQLITE_DONE) return sqlite_failure(db, rc);
    if(sqlite3_changes(db) != 1)
...
StoreResult SQLiteRunStore::register_graph(const GraphDefinitionRef& graph) {
    ...
    auto* db = database(db_);
    Statement query(db, "SELECT definition_digest,compatibility_class FROM graph_definitions "
                        "WHERE template_id=? AND revision=?");
    bind_text(query.get(), 1, graph.template_id);
    bind_text(query.get(), 2, graph.revision);
    if(sqlite3_step(query.get()) == SQLITE_ROW) {
        const bool same = column_text(query.get(), 0) == graph.definition_digest &&
                          column_text(query.get(), 1) == graph.compatibility_class;
        return {same ? StoreStatus::Committed : StoreStatus::RevisionConflict, 0,
                same ? std::string() : "graph revision digest mismatch"};
    }
    Statement insert(db, "INSERT INTO graph_definitions(template_id,revision,definition_digest,"
                         "compatibility_class) VALUES(?,?,?,?)");
    bind_text(insert.get(), 1, graph.template_id); bind_text(insert.get(), 2, graph.revision);
    bind_text(insert.get(), 3, graph.definition_digest); bind_text(insert.get(), 4, graph.compatibility_class);
    const int rc = sqlite3_step(insert.get());
...
std::optional<GraphDefinitionRef> SQLiteRunStore::load_graph(std::string_view template_id,
    ...
    Statement query(db, "SELECT definition_digest,compatibility_class FROM graph_definitions "
                        "WHERE template_id=? AND revision=?");
    bind_text(query.get(), 1, template_id); bind_text(query.get(), 2, revision);
    ...
    return GraphDefinitionRef{std::string(template_id), std::string(revision),
                              column_text(query.get(), 0), column_text(query.get(), 1)};
}
...
StoreResult SQLiteRunStore::schedule_timer(const DurableTimer& timer) {
    ...
    auto* db = database(db_);
    Statement insert(db, "INSERT INTO durable_timers(timer_id,run_id,due_unix_ms,payload_json)"
                         " VALUES(?,?,?,?)");
    bind_text(insert.get(), 1, timer.timer_id); bind_text(insert.get(), 2, timer.run_id);
    sqlite3_bind_int64(insert.get(), 3, timer.due_unix_ms);
    bind_text(insert.get(), 4, timer.payload.dump());
    const int rc = sqlite3_step(insert.get());
...
std::vector<DurableTimer> SQLiteRunStore::claim_due_timers(std::int64_t now_unix_ms,
    ...
    auto* db = database(db_);
    Transaction transaction(db);
    Statement query(db, "SELECT timer_id,run_id,due_unix_ms,payload_json FROM durable_timers "
    ...
    while(sqlite3_step(query.get()) == SQLITE_ROW) {
        result.push_back({column_text(query.get(), 0), column_text(query.get(), 1),
            sqlite3_column_int64(query.get(), 2), json::parse(column_text(query.get(), 3)),
            std::string(owner), now_unix_ms + lease_ms, false});
    }
    Statement update(db, "UPDATE durable_timers SET owner=?,lease_until_unix_ms=? "
    ...
    for(const auto& timer : result) {
        ...
        bind_text(update.get(), 1, owner); sqlite3_bind_int64(update.get(), 2, now_unix_ms + lease_ms);
        bind_text(update.get(), 3, timer.timer_id); sqlite3_bind_int64(update.get(), 4, now_unix_ms);
...
StoreResult SQLiteRunStore::complete_timer(std::string_view timer_id, std::string_view owner) {
    std::lock_guard lock(mutex_);
    auto* db = database(db_);
    Statement update(db, "UPDATE durable_timers SET completed=1 WHERE timer_id=? AND owner=? AND completed=0");
    bind_text(update.get(), 1, timer_id); bind_text(update.get(), 2, owner);
    const int rc = sqlite3_step(update.get());
    if(rc != SQLITE_DONE) return sqlite_failure(db, rc);
