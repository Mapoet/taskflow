...
int main() {
    ...
    const auto objects=(std::filesystem::temp_directory_path()/"phase4-object-store");std::filesystem::remove_all(objects);{
      distributed::FilesystemObjectStore store(objects,1024);std::string object_error;auto ref=store.put("tenant-a",std::string("artifact\0bytes",14),"application/octet-stream",{},&object_error);assert(ref&&ref->digest.rfind("sha256:",0)==0&&ref->size==14);auto replay=store.put("tenant-a",std::string("artifact\0bytes",14),"application/octet-stream",ref->digest,&object_error);assert(replay&&replay->digest==ref->digest);auto loaded=store.get(*ref,&object_error);assert(loaded&&*loaded==std::string("artifact\0bytes",14));assert(!store.put("tenant-a","different","text/plain",ref->digest,&object_error));auto second=store.put("tenant-a","second","text/plain",{},&object_error);assert(second);auto page=store.list("tenant-a",{},1);assert(page.objects.size()==1&&!page.complete&&!page.next_cursor.empty());auto next=store.list("tenant-a",page.next_cursor,1);assert(next.objects.size()==1&&next.complete);assert(store.list("tenant-b").objects.empty());assert(store.remove(*second).status==distributed::ObjectRemoveStatus::Removed);assert(store.remove(*second).status==distributed::ObjectRemoveStatus::NotFound);auto other=store.put("tenant-b",std::string("artifact\0bytes",14),"application/octet-stream",{},&object_error);assert(other&&other->digest==ref->digest);const auto object_path=objects/"tenant-a"/ref->digest.substr(7,2)/ref->digest.substr(7);{std::ofstream corrupt(object_path,std::ios::binary|std::ios::trunc);corrupt<<"tampered-value";}assert(!store.get(*ref,&object_error));assert(store.remove(*ref).status==distributed::ObjectRemoveStatus::Conflict);
    }
