...
namespace agent_framework::example
{
        ...
        std::optional<std::uint64_t> expected_task_revision = std::nullopt)
    {
        ...
        if (!pending_clarification)
        {
                ...
                invocation_now_ms <= latest->expires_at_ms)
            {
                ...
                const auto checkpoint = store.load_turn(identity, latest->turn_id);
                if (!checkpoint)
                    throw std::runtime_error("clarification_replay_turn_missing");
                ...
                for (const auto &message : store.messages(identity))
                    if (message.turn_id == latest->turn_id && message.role == "assistant")
        ...
        if (!pending_clarification && runtime.task_classifier && !control_only)
        {
            ...
            *classification = conversation::apply_task_routing_policy(
            ...
            {
                intent = conversation::task_input_intent(
                    classification->intent, active_task.has_value());
                control_only = intent == conversation::TaskInputIntent::StatusQuery ||
                               intent == conversation::TaskInputIntent::CancelTask ||
                               intent == conversation::TaskInputIntent::SuspendTask;
            }
        }
        ...
        if (intent == conversation::TaskInputIntent::StatusQuery)
        {
            ...
            const std::string response = status.ok
                                             ...
                                             : nlohmann::json{{"found", false}, {"task_id", request.task_id}, {"error", status.error}}.dump(2);
            conversation::ConversationEngine control_engine(
                ...
                return outcome; }, std::move(event_sink));
        ...
        if (!pending_clarification && !needs_clarification)
        {
            ...
            if (!task_open.ok)
                throw std::runtime_error(task_open.error);
            if (!request.classification_decision_id.empty())
            {
                const auto annotated = task_registry.annotate_run_decisions(
                    request.identity, request.task_id, request.run_id,
                    request.classification_decision_id, request.clarification_id);
                if (!annotated.ok)
                    throw std::runtime_error(
                        "task_run_decision_annotation_failed:" + annotated.error);
            }
    ...
    inline ExecutionTrustProfile execution_trust_profile_from_env()
    {
        ...
        if (value == "demo")
            return ExecutionTrustProfile::Demo;
        if (value == "test")
            return ExecutionTrustProfile::Test;
        if (value == "production")
            return ExecutionTrustProfile::Production;
    ...
    inline SkillUiStatus skill_ui_status(const std::shared_ptr<SkillServices> &services)
    {
        ...
        if (services->manager)
        {
            ...
            if (active != status.end() && active->is_string())
                result.active = active->get<std::string>();
        }
    ...
    {
        if (env_truthy("AGENT_SKILLS_DISABLED"))
            return nullptr;
    ...
    inline BootstrapResult bootstrap_agent_services(ToolBus &bus, const BootstrapOptions &options)
    {
        ...
        if (!result.skills)
            result.diagnostics.emplace_back("no skill roots were discovered");
    ...
    {
        if (std::getenv("AGENT_FS_ROOT") != nullptr)
            return;
    ...
    inline void register_live_demo_tools(ToolBus &bus)
    {
        bus.register_local_tool("add", [](const nlohmann::json &value)
                                { return nlohmann::json{{"result", value.at("a").get<int>() + value.at("b").get<int>()}}; }, live_demo_add_tool_meta());
    }
    ...
    inline LiveRuntime build_live_runtime(const LiveRuntimeOptions &options)
    {
        if (options.agent_name.empty())
            throw std::invalid_argument("LiveRuntimeOptions.agent_name is required");
        if (!options.provider.empty())
            set_environment_override("AGENT_LLM_PROVIDER", options.provider);
        if (!options.model.empty())
            set_environment_override("AGENT_LLM_MODEL", options.model);
            ...
            runtime.trust_profile != ExecutionTrustProfile::Production)
        {
            ...
            {
                return conversation::TaskCommandPrincipal{"local-interactive-user", identity, {"task:read", "task:write", "task:control"}, true};
            };
        }
        if (runtime.trust_profile == ExecutionTrustProfile::Production &&
            runtime.explicit_legacy_fallback)
            ...
            strategy && std::string(strategy) == "structured")
        {
            ...
            if (const char *timeout = std::getenv("AGENT_MEMORY_SUMMARY_TIMEOUT_MS"))
            {
                ...
                if (parsed > 0)
                    summary_profile.http_timeout_sec = std::max(1, parsed / 1000);
            }
        ...
        if (runtime.skills)
            register_skill_discovery_tool(*runtime.toolbus, runtime.skills);
        ...
        runtime.composition_report = {{"profile", runtime.trust_profile == ExecutionTrustProfile::Production ? "production" : runtime.trust_profile == ExecutionTrustProfile::Test ? "test"
                                                                                                                                                                                   : "demo"},
                                      {"toolbus", true},
                                      {"skills", runtime.skills != nullptr},
                                      {"runner_local", bool(runtime.skill_runners->resolve(agent_template::SkillRunnerKind::LocalCapability))},
                                      {"runner_process", bool(runtime.skill_runners->resolve(agent_template::SkillRunnerKind::SandboxedProcess))},
                                      {"runner_cli", bool(runtime.skill_runners->resolve(agent_template::SkillRunnerKind::Cli))},
                                      {"runner_mcp", bool(runtime.skill_runners->resolve(agent_template::SkillRunnerKind::Mcp))},
                                      {"runner_child", bool(runtime.skill_runners->resolve(agent_template::SkillRunnerKind::ChildAgent))},
                                      {"runner_nested", bool(runtime.skill_runners->resolve(agent_template::SkillRunnerKind::NestedWorkflow))},
                                      {"runner_approval", bool(runtime.skill_runners->resolve(agent_template::SkillRunnerKind::HumanApproval))},
                                      {"harness_ready", runtime.harness_ready},
                                      {"long_task_ready", bool(runtime.long_task_executor)},
        ...
        if (const char *model = std::getenv("AGENT_LLM_MODEL"))
            runtime.config.model_config.model_name = model;
        if (options.max_iterations > 0)
            runtime.config.max_iterations = options.max_iterations;
    ...
    {
        return {{"type", skill_event_type_cstr(event.type)}, {"code", event.code}, {"skillId", event.skill_id}, {"resourceId", event.resource_id}, {"taskId", event.task_id}, {"runId", event.run_id}, {"attempt", event.attempt}, {"iteration", event.iteration}, {"details", event.details}};
    }
