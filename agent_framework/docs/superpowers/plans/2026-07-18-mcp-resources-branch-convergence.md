...
- [ ] Create `skills` from `upgrade/taskflow-4x`.
- [ ] Merge `2skills` with a merge commit.
- [ ] Preserve Stage 11 presentation/activity state and Stage 12 Skills commands/options.
- [ ] Preserve `--demo-state`, `--skills-root`, authoring, and `--no-skills`.
- [ ] Verify tracked `node_modules` remain present.
- [ ] Run `git diff --check` and inspect the merge diff.

### Task 6: Combined validation and cleanup

- [ ] Build TUI, Web, and ImGui demos.
- [ ] Run MCP, resource, Skill, Cursor import, launcher, static UI, and presentation tests.
- [ ] Launch the merged TUI in the real terminal and capture a screenshot showing the combined layout and Skills/Resources state.
- [ ] Commit any merge-resolution fixes.
