...
5. **LTW4 — Typed Execution Adapters `[x]`**：production origin gate 和版本/部署代次 pin 已覆盖 Local/ToolBus、Bubblewrap、HTTP 以及原生 remote protocol adapter。统一 `ExecutionHandle` 现持久化 external operation、provider session、remote task 与 peer identity；Worker 将这些 identity 与 fencing 原子写入 Invocation，restart attach 不再依赖进程内 handle。`RemoteProtocolExecutionAdapter` 只允许 MCP/A2A 类型，强制实现 start/attach/query/cancel/escalate/reconcile，缺失 pinned session/task/peer 时 fail closed。Bubblewrap provider 跟踪真实进程组并支持 TERM/KILL，adapter 将 cooperative→terminate→kill 映射到 provider signal；不支持的 provider 能力继续显式拒绝并进入 reconciliation/ManualReview。专项测试覆盖原生 remote identity、attach、cancel 和 durable reload。
6. **LTW5 — Effect/Artifact Commit Coordination `[x]`**：`InvocationCommitCoordinator` 关联 Invocation Store、Tool Effect Journal、ObjectStore、RunStore 和 production-gated CrossStoreCoordinator。新增独立 durable input reservation，在 artifact/effect 执行前锁定 request digest、idempotency、fencing 与 replay policy；冲突在任何产物写入前被拒绝。候选输出执行 canonical digest、对象写入后回读验证、effect Started→Completed→Committed、Run effect commit、CrossStore confirm、receipt upsert 与 Invocation EffectCommitted。所有主要边界提供 fault injection；`sweep_orphans` 遍历 durable effect journal，只有解析出稳定输入时才 reconcile，输入丢失或 unknown 非幂等 effect 一律 ManualReview。Production dependencies 已强制 CrossStoreCoordinator 的 Run/Harness/Approval/Memory/Assurance participants 齐备，测试 participant 不能满足 production-ready gate。专项测试覆盖 reservation 幂等/冲突、reservation 后 crash、orphan sweep fail-closed、重复提交和 unknown effect。
...
8. **LTW7 — Incremental Result/ObjectStore `[x]`**：tenant-scoped `IncrementalResultStore`/SQLite durable registry 在不可变 CAS 上实现六类输出的确定性分片、revision/parent manifest、append 幂等、head CAS、seal/abort、重启恢复、去重和 chunk/manifest 完整性验证。stateful redactor 只持久化规则 ID、前缀长度和 policy revision，支持跨 append、跨重启匹配，seal 时将未决前缀安全替换，原始 secret 不进入 CAS/SQLite。bounded result view 已接入 `RoleRuntimeLongTaskModel`，Worker observation 以 canonical `PartialResultRef` 关联 Invocation。Filesystem 与 Remote ObjectStore 均支持 tenant-scoped bounded pagination 和回读校验 remove；规范化引用边、对象枚举 reconciliation、grace/dry-run physical GC、tombstone、pin/head 防误删和删除指标已落地。测试覆盖分页、租户隔离、完整性条件删除、跨 append/restart redaction、seal pending mask、orphan 对账、dry-run/physical GC 及全链路回归。
...
response/pipeline/verification 三轴。task-command identity/scope/revision action contract 与
ProcessLive reconnect/SIGKILL/restart campaign 已闭合；仍开放的不是 Offline correctness，而是
ProviderLive 5–10 分钟真实 provider/MCP late-result/reattach、外部 IdP/KMS 与统计充分的长时认证。
...
4. **AF-CC-LTW8**：完成 backpressure 以及 provider/tool/sandbox/MCP/A2A/child task 的 deadline/cancel/kill/reconcile 闭环；
