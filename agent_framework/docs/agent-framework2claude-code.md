...
| AF-CC8 质量/Live | `[~]` | Phase 4 offline 76/76、Phase 3 22/22、A2A 4/4、五 demo 构建通过；已有 Role Live Certification、production bundle/signature/attestation/approval/runner 与 fail-closed negative/restart 测试 | 认证框架存在不等于生产环境已认证；真实 provider/MCP/IdP/KMS/Sandbox 证据与 mandatory live matrix 尚未关闭，不能以 fixture/offline pass 替代 |
| AF-CC-LTW 长时复杂工具工作流 | `[~]` | LTW0–LTW3 已完成；LTW4 已形成 production-gated typed adapter registry、Local/ToolBus、Bubblewrap、MCP(kind)、HTTP attach/query/cancel/reconcile 和 A2A ChildTask adapter，并持久化 adapter revision/generation/external operation/restart policy；LTW5 已形成 Invocation/Effect/Object/Run/CrossStore commit coordinator、对象回读校验、receipt upgrade、Unknown 非幂等 ManualReview | LTW4 的 Bubblewrap 强杀、A2A 远端 task-id attach、MCP 原生 session 恢复仍未闭环；LTW5 尚缺将真实 Harness participant 纳入同一生产 composition 与全写点 crash injection；LTW6–LTW9 尚缺，现状仍不能声称已具备生产级长时自主工作能力 |
