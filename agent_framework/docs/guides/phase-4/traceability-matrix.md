...
| `R4V2-UIX` Interaction UI graph | `P4-UIXR0–P4-UIXR8 / phase4-uix-r2` | local-closed / external-partial | `ui/{interaction_graph,interaction_projection_store,interaction_source_adapters,production_interaction_assembler,ui_manager,presentation_model}`；Web/TUI/ImGui Interaction UI | targeted 8/8；direct Store join negative tests；SQLite restart/replay/orphan；Web static PASS；三 UI build；真实 Chrome runtime screenshot | OIDC/JWT/KMS、cross-host replay/HA、production Live golden campaign pending |
