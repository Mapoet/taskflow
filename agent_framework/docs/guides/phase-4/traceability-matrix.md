...
| `R4V2-04` 多角色专业验收 | `P4-V2-F4V.1–F4V.10` | partial | `assurance/{professional_workflow,assurance_graph_template,evidence,arbiter}`；`RoleRuntimeAssuranceModel`；`ManifestEvidenceOracle`；InMemory/SQLite `AssuranceStore` | `phase4_assurance_workflow_{contracts,negative,integration}`、`phase4_assurance_workflow`：strict/CAS、Verification Planner、五专业角色+Resolver、能力交集/独立性、evidence closure、oracle precedence、缺产物、自证降权、冲突、RoleRuntime、restart、GraphExecutor；报告 `acceptance-P4-V2-F4V-20260810.md` | 离线核心 implemented；真实 repo/build/runtime/domain/security/Sandbox oracle adapters、双 verifier 策略、生产 calibration/benchmark、跨 Store atomic、修复后自动复验、默认强制、Live/UI pending，故不升级为 verified |
...
| `R4V2-RC-06` Production Live | `R6L-R0–R6 / phase4-r6l-production-live-r1` | blocked / executing | 获批 Environment/Matrix；真实 mandatory role/dependency cells；durable independent attestation；accountable ApprovalStore decision；非对称/KMS signature；未过期、无 blocker 的 `executed=true` 报告 | R0/R1/R3/R4：strict bundle、SQLite immutable attestation/restart verifier、Store-backed approval binding、Ed25519 required gate，targeted 4/4、offline 61/61；R2/R5/R6 仍缺真实多 provider/IdP/MCP/A2A/Sandbox、scheduler、实际 KMS signer/approval，禁止签发认证 |
