...
| LLM Observability | `[~]` | 64–70% | Role Runtime 已记录 provider/model/profile/prompt/view/route/fallback/token/cost/latency/calibration manifest 并桥接 span/audit；P4-PC3 在 production Harness 强制 invocation evidence，输出 GenAI span 与 duration/token/cache/cost metrics，关联 stage、trace/span 和 memory pin，缺失/导出失败 fail closed | OTLP mTLS/SLO dashboard、真实 provider usage/cost 对账、跨进程 trace 完整性、sampling/cardinality 压测和 live evidence |
