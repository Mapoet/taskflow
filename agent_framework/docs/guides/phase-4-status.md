...
| WP4.7 Live Certification | `[~]` | 72–77% | typed Environment/Profile/Matrix/Cell/Checkpoint/Report；RoleRuntime invocation manifest 绑定；角色独立、只读/blind/strong-oracle；SQLite CAS/restart；R6L strict production bundle；WAL/FULL immutable attestation Store 与跨重启独立 verifier；Store-backed approval digest/SoD/scope/expiry binding；Ed25519 public verification；显式 no-skip required gate；61/61 offline | 可部署 production runner/外部 scheduler、真实多 provider 与 IdP/MCP/A2A/Sandbox 全矩阵、实际 KMS signer、获批 production decision 和未过期 `executed=true` 报告仍未完成 |
