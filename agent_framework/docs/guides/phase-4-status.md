...
| WP4.2 Durable Run | `[~]` | 89–91% | SQLite RunStore、状态机、CAS；P4-DRA 原子 checkpoint/graph cursor/event/effect/memory pin/interruption；摘要链 replay/time-travel；durable timer/restart；`DurableRunCoordinator`；effect CAS/reconcile；Run/Harness saga journal、每个 Harness checkpoint 自动 observer/reconcile、双 revision/digest pin | Approval/Memory/Assurance 跨 Store 原子协调与更完整 production crash matrix |
