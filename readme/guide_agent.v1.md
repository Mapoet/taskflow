...
系统采用模块化设计，从输入到输出共分为若干层，如下所示：

-   **输入层**: 包含系统提示词（System Prompt）、用户输入（User Prompt）和对话记忆（Memory）。通过不同的源节点 (Source) 提供给工作流。
-   **LLM 层**: 负责向多模态大模型请求推理。LLM Node 接收提示词、可用工具描述以及上下文信息，返回计划、工具调用列表、中间推理以及最终答复。
-   **解析层**: 将 LLM 的输出解析成结构化的调用规范 (CallSpec)。
-   **循环执行层**:使用循环节点构建代理的决策迭代，包括工具调用、知识库检索和后续 LLM 调用。
-   **工具调用层**:通过并行算法节点批量调用工具，并可扩展多种类型（本地函数、MCP服务、外部 API）。
-   **知识检索层**: 针对工具返回的线索或 LLM 需要的信息执行多模态检索或数据库查询[7](https://kanerika.com/blogs/multimodal-rag/#:~:text=What%20is%20Multimodal%20RAG%3F)。
-   **聚合层**: 将本轮工具和检索结果合并，生成供下一轮 LLM 或最终输出使用的综合结果。
-   **输出层**: 将最终答复通过 CLI、ImGui 或 Web 向用户呈现，支持流式输出。

这一层次化结构使得每个模块可以独立开发和测试，并通过 `workflow` 自动推断连接关系。
...
以下步骤展示如何使用 Taskflow `workflow` 构建完整的代理工作流图。示例代码片段采用伪代码/泛型模板形式：

#### 6.1.1 创建源节点

源节点负责注入初始数据。在每个会话开始时，创建系统提示词、用户输入、可用工具描述和记忆等。示例：
```c++
    using wf = workflow;
    wf::GraphBuilder builder;

    // 系统提示词
    auto [sys_node, tSys] = builder.create_typed_source<std::string>(
        "SystemPrompt", /*output_keys*/ {"prompt"},
        [](auto& output){ output["prompt"] = "你是一名资深航天专家，…"; }
    );

    // 用户输入
    auto [user_node, tUser] = builder.create_typed_source<std::string>(
        "UserInput", {"query"},
        [](auto& out){ out["query"] = get_user_question(); }
    );

    // 对话记忆（可用 Any 节点）
    auto [mem_node, tMem] = builder.create_any_source(
        "Memory", {"context"},
        [](auto& out){ out["context"] = retrieve_short_term_memory(); }
    );

    // 工具列表（从 ToolBus 获取）
    auto [tools_node, tTools] = builder.create_any_source(
        "ToolList", {"tools"},
        [&bus](auto& out){ out["tools"] = bus.export_as_llm_tools(); }
    );
```
#### 6.1.2 创建 LLM 节点

LLM 节点需要多个输入：系统提示词、用户输入、记忆、工具描述、图像或音频。可以通过 `create_any_node` 创建，functor 中调用 `invoke_llm` 并分发流式输出：

```c++
    auto [llm_node, tLLM] = builder.create_any_node(
        "LLM", 
        { {"SystemPrompt","prompt"}, {"UserInput","query"},
          {"Memory","context"}, {"ToolList","tools"},
          {"ImageInput","data"}, {"AudioInput","data"} },
        [on_stream](const auto& inputs){
            LLMInput in;
            in.system_prompt = std::any_cast<std::string>(inputs.at("prompt"));
            in.user_prompt = std::any_cast<std::string>(inputs.at("query"));
            in.context = std::any_cast<std::string>(inputs.at("context"));
            in.tools = std::any_cast<std::vector<nlohmann::json>>(inputs.at("tools"));
            if (inputs.count("data")) { /* 处理多模态数据 */ }
            // 调用模型
            auto out = invoke_llm(in, on_stream);
        ...
        // output keys 与返回字典键一致
        {"tool_calls","reasoning","is_final","final_answer","audio_out"}
    );
```

`on_stream` 回调会在 LLMNode 生成每个 token 时触发，将 token 发送到 UI。此处可调用 SSE、WebSocket 或 CLI 输出函数。
...
`create_for_each` 的第三个参数可以指定初始共享对象（如上例中的 `{"results", vector}`），每个任务都能读写该共享变量。需要注意多线程安全问题；如果共享对象是 vector，应使用 mutex 或使用 atomic 操作，也可以在 `workflow`的并行节点中返回局部结果再通过 reduce 聚合。
